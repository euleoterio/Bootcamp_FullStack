...
async function getStatesWithMoreOrLessCities(more) {
  ...
  const result = [];
  if (more) {
    list.slice(0, 5).forEach((it) => result.push(it.uf + ' - ' + it.count));
  } else {
    list.slice(-5).forEach((it) => result.push(it.uf + ' - ' + it.count));
  }
  console.log(result);
}
...
async function getBiggerOrSmallerNameCities(bigger) {
  ...
  for (state of states) {
    let city;
    if (bigger) city = await getBiggerName(state.Sigla);
    else city = await getSmallerName(state.Sigla);
